...
func TestSuggsRepo_SuggestStreet(t *testing.T) {
	...
	// good query
	qArgs := &models.SuggestStreetRepoReq{Street: "Измайл", CityId: 1, SuggsLimit: 5}
	mock.
	...
	suggs, err = repo.SuggestStreet(&models.SuggestStreetRepoReq{Street: "Измайл", CityId: 1, SuggsLimit: 5})
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("there were unfulfilled expectations: %s", err)
		return
	}
	...
	_, err = repo.SuggestStreet(&models.SuggestStreetRepoReq{Street: "Измайл", CityId: 1, SuggsLimit: 5})
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("there were unfulfilled expectations: %s", err)
		return
	}
...
func TestSuggsRepo_SuggestHouse(t *testing.T) {
	...
	// good query
	qArgs := &models.SuggestHouseRepoReq{House: "7", StreetId: 1, SuggsLimit: 5}
	mock.
		ExpectQuery(`SELECT house FROM houses WHERE street_id ...`).
		WithArgs(qArgs.StreetId, qArgs.House+"_%", 5).
		WillReturnRows(rows)
...
func TestSuggsRepo_GetStreet(t *testing.T) {
	...
	// good query
	qArgs := &models.GetStreetRepoReq{Street: "Измайловский прос", CityId: 1}
	mock.
		ExpectQuery(`SELECT id, name FROM streets WHERE city_id...`).
		WithArgs(qArgs.CityId, qArgs.Street, "").
		WillReturnRows(rows)
	...
	expectResp := &models.GetStreetRepoResp{Name: "Измайловский проспект", StreetId: 0}

	if !reflect.DeepEqual(suggs, expectResp) {
		t.Errorf("results not match, want %v, have %v", expectResp, suggs)
		return
	}
